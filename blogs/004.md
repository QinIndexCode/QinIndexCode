...
categories: [AI, Node.js]
tags: [AI Agent, Node.js, LangChain, OpenAI]
...
步骤1：设置环境
创建 agent.js 文件，导入库：
JavaScriptconst { openai } = require('@ai-sdk/openai');
const { generateText, tool } = require('ai');
const fetch = require('node-fetch');
const z = require('zod');
步骤2：定义工具
工具是代理的核心，用于外部交互。这里定义一个天气查询工具：
JavaScriptconst getWeather = tool({
...
JavaScriptasync function activityAgent(city) {
...
结论
在 Node.js 中构建 AI Agent 不仅可行，还非常实用。它让 JS 开发者能快速进入 AI 领域，创建智能应用。无论你是构建聊天机器人还是自动化工具，都值得一试。如果你有 OpenAI 密钥，现在就试试上面的代码吧！欢迎在评论区分享你的经验。
